...
class TestDeltaR2(unittest.TestCase):
    ...
    def test_single_transitions(self):
        ...
        self.assertAlmostEqual(metrics.delta_r2(r_traj).numpy(), 0.01)
    ...
    def test_single_trajectory(self):
        for dt in range(1, 10):
            ...
            val = metrics.delta_r2_eq_nobd(r_traj_tp1, dt=dt).numpy()
            val += metrics.delta_r2_eq_nobd(r_traj_tp2, dt=dt).numpy()

            self.assertAlmostEqual(metrics.delta_r2(r_traj, b_traj, dt=dt).numpy(), val)
    ...
    def test_two_trajectory(self):
        for dt in range(1, 10):
            ...
            val = metrics.delta_r2_slow_exact(r_traj, b_traj, i_traj, dt=dt).numpy()
            self.assertAlmostEqual(metrics.delta_r2(r_traj, b_traj, i_traj, dt=dt).numpy(), val)
    ...
    def _test_random_walks(self):
        for seed in range(10):
            ...
            for dt in range(1, 10):
                val = 0
                for i in range(10):
                    val += metrics.delta_r2(r_traj[i_traj == i], b_traj[i_traj == i], dt=dt)
                self.assertAlmostEqual(metrics.delta_r2(r_traj, b_traj, i_traj, dt=dt).numpy(), val.numpy())
...
class TestMetrics(unittest.TestCase):
    def test_2f4k(self):
        ...
        print()
        print(metrics.low_bound_delta_r2_eq(r_traj, b_traj).numpy())
        print(metrics.mse(r_traj, b_traj).numpy())
        print(metrics.mse_eq(r_traj, b_traj).numpy())
        print(metrics.cross_entropy(r_traj, b_traj).numpy())
        print(metrics.cross_entropy(r_traj, b_traj, eps=1e-8).numpy())
